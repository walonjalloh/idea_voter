...
function Hero({handleForm,handleShow}) {
  ...
  return (
    <section className=" flex flex-col justify-center items-center bg-hero-image bg-cover bg-no-repeat bg-center h-[600px]">
        ...
        <div>
          <button className="border border-black bg-black text-white/70 px-4 py-2 lg:text-3xl rounded mx-2" onClick={handleForm}>Submit Your Idea</button>
          <button className='border border-black bg-black text-white/70 px-4 py-2 lg:text-3xl rounded' onClick={handleShow} >View Ideas </button>
        </div>
    </section>
  )
}
